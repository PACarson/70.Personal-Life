...
function createTriggers() {
  ...
  // 被触发"，不需要额外手动操作）。
  // 【2026-08-27 新增 consumeReminderRequests】见
  // 00_Implementation_Plan_TaskReminder_EventConsumer.js——补齐
  // Personal Life OS → REMINDER_REQUESTED（entity_type: TASK）→
  // Reminder OS 这条集成缺口的消费层，只覆盖 TASK 路径。
  var handlerNames = ['checkReminders', 'checkOffsetReminders', 'consumeReminderRequests'];
  ...
  // 独立触发器、独立函数、独立 LockService 锁，不塞进
  // checkOffsetReminders 内部——避免挤占它已经调校过的
  // EXECUTION_TIME_BUDGET_MS 预算，且两者各自出问题时职责边界清楚。
  // GAS 的 everyMinutes() 不支持指定与另一个触发器的相位偏移，两个
  // 触发器各自每5分钟独立运行，最坏情况只是新登记的规则多等一个
  // ~5分钟周期才被 checkOffsetReminders 处理到，不影响正确性。
  ScriptApp.newTrigger('consumeReminderRequests').timeBased().everyMinutes(5).create();

  Logger.log('✅ Reminder OS 自己的触发器挂好了:');
  Logger.log('  checkOffsetReminders — 每5分钟（Unified Reminder Engine，Pre-Due + Overdue 两个阶段）');
  Logger.log('  consumeReminderRequests — 每5分钟（REMINDER_REQUESTED 事件消费者，仅 TASK 路径）');
